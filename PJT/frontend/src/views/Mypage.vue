<template>
  <div class="flex flex-col h-screen w-auto">
    ...
    <div class="flex flex-col justify-center items-center flex-grow bg-blue-100 w-auto pt-10 pb-10">
      ...
      <table class="w-4/6 border-collapse border-2 border-gray-400 border-opacity-50">
        <thead class="border-collapse border border-gray-400 border-b-2 bg-gray-200">
          <tr>
        ...
        <tbody class="bg-gray-100">
          ...
          >
            ...
            <td class="text-center border-collapse border border-gray-400" v-else>
              {{ review.content.substring(0, 40) }}<button class="text-blue-900">...</button>
            </td>
    ...
    del(rid) {
      delReview(
        ...
        () => {
          ...
          this.reviewList = this.reviewList.filter((element) => element.rid != rid);
        },
        () => {
          alert('오류가 발생했습니다.');
        }
